...
public class MooreMachine implements Cloneable {
	...
	public MooreMachine() {
		...
		for (int i = 0; i < states; ++i) {
			moves[i] = rand.nextInt(values.length);
		}
	}
	...
	public MooreMachine clone() {
		...
		for (int i = 0; i < nextSt.length; ++i) {
			nextSt[i] = Arrays.copyOf(nextState[i], nextState[i].length);
		}
		int[] mv = Arrays.copyOf(moves, moves.length);
	...
	public Turn getMove(int state) {
		return Turn.values()[moves[state]];
	}
